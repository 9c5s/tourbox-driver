...
Windows では、Windows 10 のバージョン 1809 以降 (Windows 11 を含む) を対象にする。
本アプリは、Windows の MIDI API として WinRT (Windows.Devices.Midi) を使う。
その理由は [ADR-0018](docs/adr/0018-WindowsのMIDIはWinRTバックエンドを使い出力ポートは機器IDで対応付ける.md) に記録している。
...
WinRT の一覧では、loopMIDI のポートの名前が loopMIDI の画面の表示と異なることがある。
入力ポートの名前は `TourBox MIDI In [1]` のように末尾に番号が付き、出力ポートの名前はどれも `MIDI` になる。
それでも設定ファイルの `output` と `input` には、loopMIDI で付けた名前 (`TourBox MIDI Out` など) をそのまま書けばよい。
入力ポートは部分一致で選ぶので、名前の末尾に番号が付いていても一致する。
出力ポートは、名前が一致するものがなければ、名前が一致する入力ポートと同じ機器の出力ポートを選ぶ。
loopMIDI のポートは、入力と出力で機器の識別子が同じになる。

`tourbox-midi list-ports` は、名前の後の括弧に機器の識別子を表示する。
次の例では、出力ポートの 1 つ目の `MIDI` が `TourBox MIDI Out` の出力、2 つ目が `TourBox MIDI In` の出力である。
識別子の値は環境によって異なる。
...
リポジトリ直下の `config.example.toml` は、すべてのコントロールに割り当てを持つ設定例である。
上の場所にコピーすれば、そのまま使える。
Note 番号と CC 番号は、DAW 側の使い方に合わせて変える。
...
`config.example.toml` の割り当ては次のとおり。
チャンネルはすべて 1 で、Note のベロシティは 127 である。

| 操作 | 送るメッセージ |
|---|---|
| Tall、Short、Top、Side の押下と解放 | Note 60、61、62、63 |
| Scroll の押込 | Note 64 |
| 十字キーの上、下、左、右 | Note 65、66、67、68 |
| C1、C2、Tour | Note 69、70、71 |
| Knob の押込、Dial の押込 | Note 72、73 |
| Knob の回転 | CC 1 (絶対) |
| Scroll の回転 | CC 2 (相対、`twos_complement`) |
| Dial の回転 | CC 3 (絶対) |
| Side を押している間の Knob の回転 | CC 11 (絶対) |
| Side を押している間の Top | Note 74 |

Side は修飾ボタンである。
Side を押している間は、Knob の回転と Top だけが Side のレイヤの割り当てに切り替わり、ほかの操作は通常の割り当てのままである。
Side 自身の押下と解放でも Note 63 を送る。

ハプティクスは 3 軸とも強、中速で、Side を押している間の Knob だけを弱にしている。
`[haptics.control]` では、チャンネル 16 の次の CC を受信する。

| CC | 切り替える対象 |
|---|---|
| 99 | 全軸のハプティクスのなしと設定値 (`master`) |
| 100 | Knob の強度 |
| 101、102 | Scroll の強度と速度 |
| 103、104 | Dial の強度と速度 |
| 110 | Side を押している間の Knob の強度 |

### セクション

書き方の例は `config.example.toml` にあり、各キーの意味と選べる値をコメントに書いてある。
セクションの役割は次のとおり。
...
出力ポートは、名前が一致するものがなければ、名前が一致する入力ポートと同じ機器の出力ポートを選ぶ (「Windows の仮想 MIDI ポート」の節を参照)。
...
- `list-ports`: MIDI の出力ポートと入力ポートの名前を一覧表示する。Windows で設定ファイルに書く名前を確かめるときに使う。Windows では名前の後に機器の識別子を表示する (表示の見方は「Windows の仮想 MIDI ポート」の節にある)。
...
Windows の従来の MIDI API (WinMM) は、ポート名を 31 文字までしか返さない。
DAW などのアプリが WinMM を使う場合、32 文字以上の名前は一覧で途中が切れる。
本アプリが使う WinRT の一覧で 32 文字以上の名前がどう表示されるかは、確認していない。
...
ポートは機器 ID で見分けるので、名前が同じ `MIDI` の出力ポートが複数あっても取り違えない。
...
このとき元の接続がそのまま使えるかは、まだ実機で確認していない (受け入れ確認の 5b で確認する)。
使えない場合、入力ポートは受信がないことと消失を区別できないので、DAW から送った CC でハプティクスが切り替わらなくなる。
送信や受信が復帰しない場合は、次のどちらかで復旧する。
...
### Windows の出力ポートの定期的な開き直し

本アプリは Windows で、出力ポートを開いてから 60 分ごとに自動で開き直す。
開き直しには数十ミリ秒かかり、その間の操作は開き直した後に送られる。
開き直しに失敗した場合は、「出力ポートがない間の操作」の節と同じく操作を送らず、5 秒ごとに開き直しを試みる。
定期的に開き直す理由は [ADR-0020](docs/adr/0020-WindowsのMIDI出力ポートは一定時間ごとに開き直す.md) に記録している。
...
| 操作 | 届くメッセージ |
|---|---|
| ボタン 14 種 (十字キーと押込を含む) の押下と解放 | チャンネル 1 の Note の On (ベロシティ 127) と Off。Note 番号は「設定例の割り当て」の節の表のとおりで、ボタンごとに異なる |
| Knob の回転 | チャンネル 1 の CC 1。時計回りで 1 ずつ増え、反時計回りで 1 ずつ減る (絶対 CC) |
| Scroll の回転 | チャンネル 1 の CC 2。上に回すと値 1、下に回すと値 127 (相対 CC) |
| Dial の回転 | チャンネル 1 の CC 3。時計回りで 1 ずつ増え、反時計回りで 1 ずつ減る (絶対 CC) |

Knob と Dial の内部値は 0 から始まる。
値を減らす方向から回すと値 0 が続けて届くので、増やす方向 (時計回り) から回す。

### 4. 修飾レイヤ (両 OS)

項目 3 の状態で、次を確認する。

1. Side を押すと Note 63 の On が届く。押したまま Knob を回すと、CC 1 ではなく CC 11 が届く。Side を離すと Note 63 の Off が届き、Knob の回転は CC 1 に戻る。
2. Side を押し、Top を押し、Side を離し、Top を離す。Top の押下で Note 74 の On が、Top の解放で Note 74 の Off が届き、Note 62 は届かない。DAW に Note 74 の On が残らない。
...
5. 5 秒以内に「MIDI 出力ポートを開きました。」と「MIDI 入力ポートを開きました。」が出ることを確認する。出力側のログの `port` は `MIDI` になり、その直前に「出力ポート "TourBox MIDI Out" に名前が一致するポートがないため、名前が一致する入力ポートと機器の識別子が同じ出力ポートを選びました。」が出る。
...
3. 送信と受信が自動で復帰することを確認する。ポートの消失と開き直しのログが出たかも記録する。
4. 受信が復帰しない場合は、そのことを記録し、設定ファイルを正しい内容のまま保存し直す (「loopMIDI を再起動したときの復帰」の節を参照)。「MIDI 入力ポートを開き直します。」と「設定ファイルを再読込して反映しました。」が出て、DAW から送った CC で手応えが変わるようになることを確認する。
5. 送信が復帰しない場合は、そのことを記録し、本アプリを起動し直すと復帰することを確認する。

### 6. 設定ファイルの自動再読込 (両 OS)

1. 項目 3 の状態で、設定ファイルの `tall = { note = 60 }` を `tall = { note = 59 }` に変えて保存する。
2. ログに「設定ファイルを再読込して反映しました。」(`changed=map`) が出て、Tall で Note 59 が届くことを確認する。
3. `tall = { note = 128 }` に変えて保存する。
4. ログに「設定ファイルの再読込に失敗したため、現在の設定を維持します」と、行番号付きの理由が出ることを確認する。本アプリが終了せず、Tall で Note 59 が届き続けることを確認する。
5. 設定ファイルを元に戻して保存する。

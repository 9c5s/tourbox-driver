...
`config.example.toml` の名前に合わせる場合は、`TourBox MIDI` と `TourBox MIDI In` の 2 つを作る。
...
### Windows での `--config` のファイル名

設定ファイルの変更は、親ディレクトリの変更通知のうち、ファイル名が一致するものだけで検知する。
この比較は大文字と小文字を区別する。
Windows では `--config CONFIG.TOML` でも `config.toml` を読み込めるが、保存しても再読込が起きない。
`--config` には、実際のファイル名と同じ大文字と小文字で書く。
...
- Windows では、loopMIDI で `TourBox MIDI` と `TourBox MIDI In` の 2 つのポートを作る。
- DAW か MIDI モニタで、出力ポート `TourBox MIDI` から届くメッセージを表示できるようにする。項目 5、5b、7 では、DAW から入力ポート `TourBox MIDI In` へ CC を送れるようにもする。
...
3. DAW か MIDI モニタで、`TourBox MIDI` を入力に選ぶ。
...
1. 項目 3 の状態で、TourBox の操作が DAW に届くこと (送信) と、DAW から CC 100 を送ると Knob の手応えが変わること (受信、項目 7 を参照) を確認する。
2. loopMIDI を終了する。`tourbox-midi list-ports` で 2 つのポートが消えたことを確認する。
3. 10 秒以上待つ。ログに「MIDI 出力ポートが一覧から消えました。」と「MIDI 入力ポートが一覧から消えました。」が出ることを確認する。待つ間に TourBox を操作した場合は、出力側は「MIDI メッセージを送信できませんでした。」になる。
...
受信のたびに、ログに「受信した Control Change でハプティクス設定を変更します。」が出る。

1. CC 100 の値 0 を送ると Knob の手応えがなくなり、値 127 を送ると強い手応えに戻る。
2. Side を押している間の Knob の手応えは、CC 100 では変わらず、CC 110 で変わる。
3. CC 102 の値を 0、64、127 と変えると、Scroll の手応えの速度が変わる。
